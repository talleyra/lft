...
add_changepoints <- function(df,
  ...
  cpt_tbl <- tibble(nr = points_vec) %>%
    mutate("cp_{fun}" := str_c(glue::glue("cp_{fun}_"), nr))

  df_out <- df %>%
    mutate(nr = row_number()) %>%
    left_join(cpt_tbl, by = "nr") %>%
    dplyr::select(-nr) %>%
    fill(glue::glue("cp_{fun}"), .direction = "down")


  if (!is.null(last)) {
    chpts_filter <- cpt_tbl %>%
      slice_tail(n = last) %>%
      pull(glue::glue("cp_{fun}"))

    print(chpts_filter)

    df_out <- df_out %>%
      filter(!!sym(glue::glue("cp_{fun}")) %in% chpts_filter)
  }
